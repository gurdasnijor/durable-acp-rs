//! DurableStateProxy — reusable ACP proxy component.
//!
//! Intercepts all ACP messages flowing through the conductor's proxy chain
//! and persists them to a durable state stream. Follows the cookbook's
//! `reusable_components` pattern: a struct implementing `ConnectTo<Conductor>`.
//!
//! Used both in-process (via `ProxiesAndAgent::new(agent).proxy(DurableStateProxy { app })`)
//! and as a standalone binary (`src/bin/durable-state-proxy.rs`).
